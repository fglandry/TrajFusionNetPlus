...
class TrajectoryTransformerOnlySpeed(HuggingFaceTimeSeriesModel):
    ...
    def train(self,
        ...
        encoder_input_size = 1 # data_element.shape[-1]
...
class VanillaTransformerForForecast(TimeSeriesTransformerPreTrainedModel):
    ...
    def forward(
        ...
        normalized_trajectory_values = normalized_trajectory_values.unsqueeze(-1)
        #labels = labels[:,:,-1].unsqueeze(-1) if labels is not None else labels
        #normalized_trajectory_values = normalized_trajectory_values[:,:,-1].unsqueeze(-1) # only keep speed
        labels = labels[:,:,-1].unsqueeze(-1) if labels is not None else labels
...
def load_pretrained_trajectory_tf_only_speed(dataset_name: str,
    ...
    if submodels_paths_override:
        ...
        if dataset_name == "pie":
            #checkpoint = "data/models/pie/TrajectoryTransformerNoSpeed/15Feb2025-20h03m25s_SPI5"
            checkpoint = "data/models/pie/TrajectoryTransformerOnlySpeed/29Apr2025-21h15m56s_SPI11"
